module.exports = {
    getPosts: (req, res, next) => {
        ...
        db.getallpostsone(email).then(posts => {
    ...
    createPost: (req, res, next) => {
        ...
        const db = req.app.get("db")
        db.makePost([content, new Date(), user_id]).then(newPosts => {
    ...
    changePost: (req, res, next) => {
        ...
        const {content} = req.body;
        const db = req.app.get("db");
        db.editPost([content, post_id, user_id]).then(updatedPosts => {
    ...
    removePost: (req, res, next ) => {
        ...
        console.log(user_id);
        const db = req.app.get("db");
        db.deletePost(user_id).then(remainingPosts => {
